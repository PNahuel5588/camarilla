...
class AuthorizedUser(Filter):
    """Allow only messages from the configured authorized users."""

    async def __call__(self, message: Message) -> bool:
        """Return True if the sender is in AUTHORIZED_USER_IDS."""
        user_id = message.from_user.id if message.from_user else None
        if user_id is not None and user_id in config.AUTHORIZED_USER_IDS:
            return True
